...
type PaymentTab = 'nequi' | 'daviplata' | 'bancolombia' | 'paypal' | 'card_online'

export default function DonationWidget() {
  const { t } = useI18n()
  const [activeTab, setActiveTab] = useState<PaymentTab>('nequi')
  const [selectedAmountCOP, setSelectedAmountCOP] = useState<number>(25000)
  ...
  const suggestedAmountsCOP = [10000, 25000, 50000, 100000]
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {/* Montos Sugeridos COP ($10.000, $25.000, $50.000, $100.000 COP) */}
        <div className="mb-6 p-3 rounded-2xl bg-white/[0.03] border border-white/10 max-w-xl mx-auto">
          <span className="text-xs font-mono text-white/50 block mb-2 font-semibold">
            Selecciona un monto sugerido de aporte:
          </span>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {suggestedAmountsCOP.map((amt) => {
              const isSelected = selectedAmountCOP === amt && !customAmountCOP
              return (
                <button
                  key={amt}
                  type="button"
                  ...
                  className={`py-2 px-2.5 rounded-xl text-xs font-bold font-mono transition-all active:scale-95 cursor-pointer ${
                    isSelected
        ...
        {/* Pestañas de Métodos de Pago (Nequi, Daviplata, Bancolombia, PayPal, Tarjeta) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 mb-6 p-1 bg-white/5 rounded-2xl border border-white/10">
          <button
            onClick={() => setActiveTab('nequi')}
            className={`py-2.5 px-2 rounded-xl text-xs md:text-sm font-semibold transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer ${
              activeTab === 'nequi'
                ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/20 font-bold'
          ...
          <button
            onClick={() => setActiveTab('daviplata')}
            className={`py-2.5 px-2 rounded-xl text-xs md:text-sm font-semibold transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer ${
              activeTab === 'daviplata'
                ? 'bg-gradient-to-r from-red-600 to-rose-600 text-white shadow-lg shadow-red-500/20 font-bold'
                : 'text-white/60 hover:text-white hover:bg-white/5'
            }`}
          >
            <span>🔴</span>
          ...
          <button
            ...
            className={`py-2.5 px-2 rounded-xl text-xs md:text-sm font-semibold transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer col-span-2 sm:col-span-1 ${
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'nequi' && (
            ...
            >
              <div className="p-5 rounded-2xl bg-purple-950/40 border border-purple-500/30 text-left relative">
                ...
                <div className="font-mono text-2xl sm:text-3xl font-bold text-white mb-2 tracking-wider">
                  {nequiNumber}
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl text-xs font-bold bg-purple-600 hover:bg-purple-500 active:scale-95 text-white transition-all flex items-center justify-center gap-2 shadow-lg shadow-purple-600/30 cursor-pointer"
                >
            ...
            >
              <div className="p-5 rounded-2xl bg-red-950/30 border border-red-500/30 text-left relative">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl text-xs font-bold bg-red-600 hover:bg-red-500 active:scale-95 text-white transition-all flex items-center justify-center gap-2 shadow-lg shadow-red-600/30 cursor-pointer"
                >
                  {copiedField === 'daviplata' ? '✅ ¡Copiado!' : `📋 ${t.donate_copy} Daviplata`}
                </button>
              </div>
              <p className="text-xs text-white/40">Abre tu app Daviplata, ingresa el número de celular y envía tu donación.</p>
            </motion.div>
          )}

          {/* TAB 3: TRANSFERENCIA BANCOLOMBIA */}
          {activeTab === 'bancolombia' && (
            <motion.div
              key="bank"
              initial={{ opacity: 0, scale: 0.98 }}
              ...
              exit={{ opacity: 0, scale: 0.98 }}
              transition={{ duration: 0.2 }}
              className="p-6 rounded-2xl bg-yellow-950/30 border border-yellow-500/30 text-left space-y-4 max-w-lg mx-auto"
            >
              <div className="flex items-center justify-between border-b border-yellow-500/20 pb-3">
                ...
                <span className="text-xs bg-yellow-500/20 text-yellow-200 px-2.5 py-0.5 rounded-full border border-yellow-500/30 font-semibold">
                  {accountType}
                </span>
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl text-xs font-bold bg-yellow-500 hover:bg-yellow-400 active:scale-95 text-black transition-all flex items-center justify-center gap-2 shadow-lg shadow-yellow-500/20 cursor-pointer"
              >
                {copiedField === 'bancolombia' ? '✅ ¡Copiado!' : `📋 ${t.savings_copy || 'Copiar Número de Cuenta'}`}
              </button>
            </motion.div>
          )}
          ...
          {activeTab === 'paypal' && (
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.98 }}
              transition={{ duration: 0.2 }}
              className="p-6 rounded-2xl bg-blue-950/30 border border-blue-500/30 text-center space-y-4 max-w-lg mx-auto"
            >
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {[5, 10, 25, 50, 100].map((amt) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold font-mono transition-all active:scale-95 cursor-pointer ${
              ...
              {/* Enlace Directo a PayPal.me/JesusBarriosGiraldo y Copiar */}
              <div className="grid sm:grid-cols-2 gap-3 pt-1">
                <a
                  href="https://paypal.me/JesusBarriosGiraldo"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3 rounded-xl text-xs font-bold bg-blue-600 hover:bg-blue-500 active:scale-95 text-white transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-600/30 font-mono"
                >
                  <span>🌐 Abrir PayPal.me/JesusBarriosGiraldo ↗</span>
                </a>

                <button
                  type="button"
                  onClick={() => copyToClipboard('https://paypal.me/JesusBarriosGiraldo', 'paypal_link')}
                  className="w-full py-3 rounded-xl text-xs font-semibold bg-white/5 hover:bg-white/10 active:scale-95 text-white/90 border border-white/10 transition-all flex items-center justify-center gap-2 font-mono cursor-pointer"
                >
                  <span>{copiedField === 'paypal_link' ? '✅ ¡Copiado!' : '📋 Copiar Enlace PayPal'}</span>
                </button>
              </div>
          ...
          {activeTab === 'card_online' && (
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.98 }}
              transition={{ duration: 0.2 }}
              className="p-6 rounded-2xl bg-emerald-950/30 border border-emerald-500/30 text-center space-y-4 max-w-lg mx-auto"
            >
              ...
              <button
                ...
                className="w-full py-3 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-500 active:scale-95 text-white transition-all shadow-lg cursor-pointer font-mono"
              >
                {copiedField === 'pse' ? '✅ ¡Copiado!' : `Transferir $${finalAmountCOP.toLocaleString('es-CO')} COP por PSE / Bancolombia`}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
